...
function renderFolders() {
  ...
  state.folders.forEach((folder) => {
    ...
    li.addEventListener('contextmenu', async (event) => {
      ...
      await fetch(`/api/folders/${folder.folder_id}`, {
        method: 'PUT',
  ...
  state.media.forEach((item) => {
    ...
    const locations = item.locations.map((loc) => {
      const status = loc.storage_type === 'Local'
        ...
        : '在线';
      const safePath = loc.path.replace(/</g, '&lt;').replace(/>/g, '&gt;');
      if (loc.storage_type === 'Web') {
        return `Web: <a href="${safePath}" target="_blank" rel="noopener">打开链接</a> (${status})`;
      }
      const downloadUrl = `/api/download?path=${encodeURIComponent(loc.path)}`;
      return `Local: ${safePath} (<a href="${downloadUrl}">下载</a> · ${status})`;
    }).join('<br />');
